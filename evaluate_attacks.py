...
parser.add_argument('--model-path',
                    default='/root/SCORE/trained_models/WRN34-10Swish_cifar10/weights-best.pt',
...
parser.add_argument('--attack-method', default='MIM')
...
def main():
    ...
    if args.white_box_attack:
        ...
        model_target = WideResNet(depth=34, num_classes=10 if args.dataset == 'cifar100' else 10).to(device)
        model_target.load_state_dict(torch.load(args.target_model_path))
        model_source = WideResNet(depth=34, num_classes=10 if args.dataset == 'cifar100' else 10).to(device)
