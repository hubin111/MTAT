...
#     jim = torch.gather(out, 1, tmp1[:, -1].unsqueeze(dim=1)) - torch.gather(out, 1, tmp1[:, -2].unsqueeze(dim=1)) > 1
...
#             out[i] = out[i]
#     return out
# WRN28-10Swish_cifar10s_lr0p2_TRADES4_epoch400_bs512_fraction0p7_LSE
# mymodel
# WRN28-10Swish_cifar10s_89.46-60.87
def wujian(out):
    # out = F.softmax(out, dim=1)
    tmp1 = torch.argsort(out, dim=1)[:, -2:]
    jim = torch.gather(F.softmax(out, dim=1), 1, tmp1[:, -1].unsqueeze(dim=1)) - torch.gather(F.softmax(out, dim=1), 1, tmp1[:, -2].unsqueeze(dim=1)) > 0.7
    for i in range(len(out)):
        # print(out[i])
        # temp = copy.deepcopy(out[i][tmp1[:, -1][i]].detach())
        # tempp = copy.deepcopy(out[i][tmp1[:, -2][i]].detach())
        temp = out[i][tmp1[:, -1][i]].clone() * (1 - int(jim[i]))
        tempp = out[i][tmp1[:, -2][i]].clone() * (1 - int(jim[i]))
        # print(tempp)
        # out[i][tmp1[:, -2][i]] += temp - tempp
        # out[i][tmp1[:, -1][i]] += tempp - temp
        # out[i][tmp1[:, -2][i]] += temp
...
class WideResNet(nn.Module):
    ...
    def __init__(self,
        ...
        # self.fc = RBFLogits(num_channels[3], num_classes, 1.5, 4)
    ...
    def forward(self, x):
        ...
        out1 = self.layer(out)
        out = self.relu(self.batchnorm(out1))
        ...
        # out = wujian(out)
        return out
        # # out = F.dropout(out, p=float(0.8))
        # out = self.logits(out)
        # norms = torch.norm(out, p=2, dim=-1, keepdim=True) + 1e-7
        # logit_norm = torch.div(out, norms)
        # return logit_norm
    ...
    #     return (self.relu(self.batchnorm(self.layer(self.init_conv(x)))))
