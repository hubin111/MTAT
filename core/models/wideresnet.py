...
def wujian(out):
    ...
    jim = torch.gather(F.softmax(out, dim=1), 1, tmp1[:, -1].unsqueeze(dim=1)) - torch.gather(F.softmax(out, dim=1), 1, tmp1[:, -2].unsqueeze(dim=1)) > 0.4
...
class WideResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out1 = out.view(-1, self.nChannels)
        out = self.fc(out1)
        # out = wujian(out)
        return out
