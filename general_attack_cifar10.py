...
def eval_adv_test_general(model, device, name):
    ...
    testset = CIFAR10_C(root='/root/SCORE/cifar-data', name = name, transform=transform_test)
    ...
    print('general_acc_total: ', general_acc_total/(idx+1))
...
def main():
    ...
    try:
        if 'model_state_dict' not in checkpoint:
            raise RuntimeError('Model weights not found at {}.'.format(1))
        model.load_state_dict(checkpoint['model_state_dict'])
    except:
        new = OrderedDict()
